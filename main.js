...
function Character(nombre, apellido, edad, sexo, raza, historia) {
  ...
  function crear() {
    ...
    newCharacterSheet = new Character(
      nombreCrear,
      apellidoCrear,
      edadCrear,
      sexoCrear,
...
function add(character) {
  ...
  const stats = { str: 1, dex: 1, int: 1, con: 1, stm: 1, lck: 1 };
  function misStats() {
    for (let character of elParty) {
      console.log(character);
      if (
        character.raza.toUpperCase() == "ORCO"
      ) {
        ...
        character.raza.toUpperCase() == "ELFO"
      ) {
        var frz = stats.str + 6; 
        ...
        character.raza.toUpperCase() == "HUMANO"
      ) {
        var frz = stats.str + 7; 
        var agl = stats.dex + 7; 
        var inl = stats.int + 6; 
        ...
        character.raza.toUpperCase() == "ENANO"
      ) {
        var frz = stats.str + 8; 
        var agl = stats.dex + 4; 
        var inl = stats.int + 5; 
  ...
  function datosDelJugador(){
    ...
    document.getElementById("stats").innerHTML  += `<p>Bienvenido, ${newCharacterSheet.nombre} tus stats son los siguientes:</p>`;   
    document.getElementById("stats").innerHTML  += `<p>Fuerza:       ${newCharacterStats.characterStr}</p>`;   
    document.getElementById("stats").innerHTML  += `<p>Agilidad:     ${newCharacterStats.characterDex}</p>`;   
    document.getElementById("stats").innerHTML  += `<p>Inteligencia: ${newCharacterStats.characterInt}</p>`;   
    document.getElementById("stats").innerHTML  += `<p>Constitución: ${newCharacterStats.characterCon}</p>`;   
    document.getElementById("stats").innerHTML  += `<p>Estamina:     ${newCharacterStats.characterStm}</p>`;   
    document.getElementById("stats").innerHTML  += `<p>Suerte:       ${newCharacterStats.characterLck}</p>`;   
  }

 function presentacionCharacter(){
     alert(`Bienvenido a Errepegia, ${newCharacterSheet.nombre} aquí vas a morir de maneras horribles y reviviras para morir otra vez, pero hey lo haras con amigos.`);
 }